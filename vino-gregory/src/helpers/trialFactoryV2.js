...
let TARGET_TYPE = ["full", "full", "conjunction", "single"]
...
let COLORS = ["black", "blue", "brown", "green", "orange", "pink", "purple", "red", "turquoise", "yellow" ]
...
class ExperimentFactory {
    constructor(faces, gaze_directions, grid_positions, target_types, letters, colors) {
        ...
        this.template = []
        this.featureTypeSampler =()=> new Sampler(["letter", "color"])
    ...
    constructTrialTemplate() {
        ...
        let template = perms.map(arr => ({ face: arr[0], gaze_direction: arr[1], grid_position: arr[2], trial_type: arr[3] }))



        let options = {
            consistent:new Sampler(["letter", "color"]),
            inconsistent:new Sampler(["letter", "color"])
            }
        ...
        for(let i = 0; i <template.length;i++){
            ...
            if(template[i].trial_type==="single"){

                let consistenctType = template[i].gaze_direction ==template[i].grid_position ? "consistent" : "inconsistent"

                let uniqueFeature = options[consistenctType].nextLoopSample()

                template[i].uniqueFeature = uniqueFeature
    ...
    generatePractice(){
        ...
        while(trials.length<12 && i <pool.length){
            ...
            if (faces_valid.size <6 && (template.trial_type ==="full" && !(faces_valid.has(template.face)))){
                faces_valid.add(template.face)
                trials.push(template)
            }
            if (template.trial_type !=="full" && !(faces_invalid.has(template.face)) && faces_invalid.size <6){
    ...
    _constructSingleTimeLineVariable(templateItem){
        ...
        let letters = this.letters.sampleWithoutReplacement(5)
        let colors = this.colors.sampleWithoutReplacement(5)
        let stimuli = letters.map((letter, i)=>({letter, color:colors[i]}))
        ...
        let target_type = "invalid"

        if(templateItem.trial_type==="full" ||  templateItem.trial_type==="full-filler"){
        ...
        if(templateItem.trial_type==="conjunction"){
            let [stim1, stim2] = new Sampler(grid_stimuli).sampleWithoutReplacement(2)

            selected = {color:stim1.color, letter:stim2.letter}
        ...
        if(templateItem.trial_type==="single"){

            let unique_feature = templateItem.uniqueFeature
            let matching_feature = unique_feature ==="letter" ? "color" : "letter"
            let stim = new Sampler(grid_stimuli).sampleWithoutReplacement(1)[0]


            selected = {[matching_feature]:stim[matching_feature], [unique_feature]:invalid_target[unique_feature], unique:unique_feature  }
    ...
    return {...templateItem, grid, target:selected, ...images, target_type}
    }
    ...
    generateExperiment(){
        ...
        let practice = this.generatePractice()

        let a = this.generateTimelineVariables("a")
        let b = this.generateTimelineVariables("b")
        let c = this.generateTimelineVariables("c")


        return {
            practice,
            block1:a.slice(0,48).map(a=>({...a, block:"block1"})),
            block2:a.slice(48,96).map(a=>({...a, block:"block2"})),
            block3:b.slice(0,48).map(a=>({...a, block:"block3"})),
            block4:b.slice(48,96).map(a=>({...a, block:"block4"})),
            block5:c.slice(0,48).map(a=>({...a, block:"block5"})),
            block6:c.slice(48,96).map(a=>({...a, block:"block6"})),
